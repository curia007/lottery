...
Examples:
    python pick4_mlx_ticket_model.py --csv idaho_pick4_history.csv --draw Night --tickets 5

    python pick4_mlx_ticket_model.py --csv idaho_pick4_history.csv --draw Day --tickets 10 --ticket-type exact

    python pick4_mlx_ticket_model.py --csv idaho_pick4_history.csv --draw Night --tickets 10 --ticket-type any

    python pick4_mlx_ticket_model.py --csv idaho_pick4_history.csv --draw both --tickets 5

    python pick4_mlx_ticket_model.py --csv idaho_pick4_history.csv --draw Night --tickets 20 --output pick4_night_predictions.csv
"""
...
TicketType = Literal["exact", "any", "boxed", "balanced", "hot", "overdue", "hot_overdue"]
...
def make_features_and_labels(digits: np.ndarray, window: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    Feature vector contains:
      ...
      - last draw digits as normalized features
    """
    xs: list[np.ndarray] = []
    ys: list[np.ndarray] = []
...
def cross_entropy_for_four_positions(logits, y):
    ...
    loss4 = nn.losses.cross_entropy(logits[:, 3, :], y[:, 3], reduction="mean")
...
def combo_key(ticket: tuple[int, int, int, int], ticket_type: str) -> str:
    if ticket_type in {"any", "boxed"}:
...
def balance_score(ticket: tuple[int, int, int, int]) -> float:
    ...
    odd_even = 1.0 - abs(odd_count - 2.0) / 2.0
    high_low = 1.0 - abs(high_count - 2.0) / 2.0
    unique_score = unique_count / 4.0
    sum_score = max(0.0, 1.0 - abs(total - 18.0) / 18.0)

    return float((odd_even + high_low + unique_score + sum_score) / 4.0)
...
def get_weights(ticket_type: TicketType, custom_weights: tuple[float, float, float, float, float] | None):
    ...
    defaults = {
        "exact": (0.62, 0.15, 0.10, 0.08, 0.05),
        "any": (0.50, 0.18, 0.12, 0.10, 0.10),
        "boxed": (0.50, 0.18, 0.12, 0.10, 0.10),
...
def print_ranked(rows: list[RankedTicket]) -> None:
    ...
    print("-" * 92)
    print(f"{'Rank':<6}{'Draw':<8}{'Type':<14}{'Ticket':<12}{'Score':<14}{'Model':<14}")
    print("-" * 92)
